...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    # Deconvolution layer
    deconv = tf.layers.conv2d_transpose(conv_1x1, num_classes, 4, strides=(2,2), padding='same', 
                                        kernel_initializer=tf.truncated_normal_initializer(stddev=0.01),
                                        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
      
    # Skip layer
    skip_layer1 = tf.add(deconv, conv_4)
    
    #Deconvolution layer
    deconv16 = tf.layers.conv2d_transpose(skip_layer1, num_classes, 4, strides=(2,2), padding='same', 
                                          kernel_initializer=tf.truncated_normal_initializer(stddev=0.01),
                                        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
   
    #Skip layer
    skip_layer2 = tf.add(deconv16, conv_3)
    
    output = tf.layers.conv2d_transpose(skip_layer2, num_classes, 32, strides=(8,8), padding='same', 
                                        kernel_initializer=tf.truncated_normal_initializer(stddev=0.01),
                                        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
